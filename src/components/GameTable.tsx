      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#07633d] text-white p-6 text-center overflow-hidden">
        ...
        <div className="bg-black/20 p-8 rounded-[2.5rem] backdrop-blur-md border border-white/10 w-full max-w-sm space-y-8 shadow-2xl">
          ...
          <div className="pt-6 border-t border-white/10 space-y-4">
            ...
            {showInstallBtn && (
              <button onClick={handleInstallClick} className="w-full py-4 bg-purple-600/50 hover:bg-purple-600 rounded-2xl font-black uppercase transition-colors mt-4">Instalar App</button>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="w-full flex justify-center h-48 md:h-64 relative">
            <Reorder.Group axis="x" values={localHand} onReorder={setLocalHand} className="flex -space-x-14 md:-space-x-20 pt-12 min-w-max">
              {localHand.map((card, idx) => {
                ...
                const rotation = offset * 2.5; // Flat fan
                const yPos = Math.abs(offset) * 4; // Flat curve

                return (
                  <Reorder.Item key={card.id} value={card} initial={{ y: 200, opacity: 0 }} animate={{ y: yPos, rotate: rotation, opacity: 1 }} whileHover={{ y: yPos - 60, scale: 1.15, zIndex: 100 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }} className="cursor-pointer active:cursor-grabbing" style={{ zIndex: idx }}>
